...
// The `AppNavigation` model lived here: a NavigationPath plus a Destination enum for the
// group forms, the expense forms and the template forms, and references back to two view
// models. There is one screen now, and its two sheets are local state, so there is nothing
// left to route.
